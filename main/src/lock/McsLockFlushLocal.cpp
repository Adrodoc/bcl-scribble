...
class McsLockFlushLocal : public Lock
{
private:
...
public:
    McsLockFlushLocal(const McsLockFlushLocal &) = delete;
    McsLockFlushLocal(const MPI_Comm comm = MPI_COMM_WORLD, const int master_rank = 0)
        : master_rank{master_rank},
          rank{get_rank(comm)}
    {
        // log() << "entering McsLockFlushLocal" << std::endl;
        MPI_Info info;
        ...
        MPI_Barrier(comm);
        // log() << "exiting McsLockFlushLocal" << std::endl;
    }
