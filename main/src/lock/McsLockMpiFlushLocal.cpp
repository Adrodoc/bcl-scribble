...
class McsLockMpiFlushLocal : public Lock
{
private:
    struct mcs_node
    {
...
public:
    McsLockMpiFlushLocal(const McsLockMpiFlushLocal &) = delete;
    McsLockMpiFlushLocal(const uint64_t rank = 0)
    {
        if (BCL::rank() == rank)
        {
    ...
    bool acquire_ext()
    {
        ...
        MPI_Accumulate(&null, 1, MPI_UINT64_T,
                       ...
                       MPI_REPLACE, BCL::win);
        MPI_Win_flush_local(my_node.rank, BCL::win);
    ...
    bool release_ext()
    {
        ...
        MPI_Accumulate(&b, 1, MPI_CXX_BOOL,
                       ...
                       MPI_REPLACE, BCL::win);
        MPI_Win_flush_local(successor.rank, BCL::win);
        return false;
    }
};
