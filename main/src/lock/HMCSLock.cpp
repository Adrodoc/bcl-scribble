...
public:
...
#ifdef PRINT
        printf("%" PRId64 " HMCS init, local_rank: %" PRId64 ", local_tail_rank: %" PRId64 "\n", global_rank, local_rank, local_tail_rank);
#endif

        winsize = 3 * sizeof(int64_t);
        MPI_Win_allocate(winsize, sizeof(int64_t), MPI_INFO_NULL, local_comm, &local_mem, &local_win);
        MPI_Win_allocate(winsize, sizeof(int64_t), MPI_INFO_NULL, global_comm, &global_mem, &global_win);
        local_mem[NEXT] = -1;
        local_mem[STATUS] = 0;
        local_mem[TAIL] = -1;
        global_mem[NEXT] = -1;
    ...
    void acquire()
    {
        ...
        if (predecessor != -1)
        {
            ...
            MPI_Accumulate(&local_rank, 1, MPI_INT64_T, predecessor, NEXT, 1, MPI_INT64_T, MPI_REPLACE, local_win);
        ...
        MPI_Accumulate(&wait, 1, MPI_INT64_T, local_tail_rank, STATUS, 1, MPI_INT64_T, MPI_REPLACE, global_win);
        MPI_Accumulate(&null, 1, MPI_INT64_T, local_tail_rank, NEXT, 1, MPI_INT64_T, MPI_REPLACE, global_win);
        ...
        {
            /* We didn't get lock. Add us as next at predecessor. */
            MPI_Accumulate(&local_tail_rank, 1, MPI_INT64_T, predecessor, NEXT, 1, MPI_INT64_T, MPI_REPLACE, global_win);
            ...
            do
            {
                MPI_Get_accumulate(0, 0, MPI_INT64_T, &status, 1, MPI_INT64_T, local_tail_rank, STATUS, 1, MPI_INT64_T, MPI_NO_OP, global_win);
                MPI_Win_flush(local_tail_rank, global_win);
            } while (status == WAIT);
        }
    ...
    void release()
    {
...
#ifdef PRINT
        printf("%" PRId64 " LOCAL release successor: %" PRId64 ", status: %" PRId64 "\n", global_rank, local_mem[NEXT], local_mem[STATUS]);
#endif
        ...
        if (local_successor != -1 && local_mem[STATUS] < HMCS_THRESHOLD)
        {
            ...
            MPI_Accumulate(&current_status, 1, MPI_INT64_T, local_successor, STATUS, 1, MPI_INT64_T, MPI_REPLACE, local_win);
            MPI_Win_flush(local_successor, local_win);
#ifdef PRINT
            printf("%" PRId64 " LOCAL released to successor: %" PRId64 ", status: %" PRIx64 "\n", global_rank, local_successor, current_status);
#endif
            return;
        }
        MPI_Get_accumulate(0, 0, MPI_INT64_T, &global_successor, 1, MPI_INT64_T, local_tail_rank, NEXT, 1, MPI_INT64_T, MPI_NO_OP, global_win);
        MPI_Win_flush(local_tail_rank, global_win);
#ifdef PRINT
        ...
        {
            MPI_Accumulate(&zero, 1, MPI_INT64_T, global_successor, STATUS, 1, MPI_INT64_T, MPI_REPLACE, global_win);
            MPI_Win_flush(global_successor, global_win);
#ifdef PRINT
            printf("%" PRId64 " GLOBAL released to successor: %" PRId64 "\n", global_rank, global_successor);
#endif
        }
#ifdef PRINT
        if (global_successor == -1)
...
#ifdef PRINT
        printf("%" PRId64 " LOCAL notify successor acquire global: %" PRId64 "\n", global_rank, local_successor);
#endif
        MPI_Accumulate(&acquire_global, 1, MPI_INT64_T, local_successor, STATUS, 1, MPI_INT64_T, MPI_REPLACE, local_win);
        MPI_Win_flush(local_successor, local_win);
    }
};
