...
class McsLockFlushEveryOperation : public Lock
{
private:
    struct mcs_node
    {
...
public:
    McsLockFlushEveryOperation(const McsLockFlushEveryOperation &) = delete;
    McsLockFlushEveryOperation(const uint64_t rank = 0)
    {
        // log() << "entering McsLockFlushEveryOperation" << std::endl;
        if (BCL::rank() == rank)
        {
        ...
        // log() << "exiting McsLockFlushEveryOperation" << std::endl;
    }

    ~McsLockFlushEveryOperation()
    {
        // log() << "entering ~McsLockFlushEveryOperation" << std::endl;
        BCL::barrier();
        if (tail.is_local())
