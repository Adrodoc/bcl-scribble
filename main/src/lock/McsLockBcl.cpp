...
public:
    McsLockBcl(const McsLockBcl &) = delete;
    McsLockBcl(const uint64_t rank = 0)
    {
        // log() << "entering McsLockBcl" << std::endl;
        if (BCL::rank() == rank)
        {
        ...
        // log() << "exiting McsLockBcl" << std::endl;
    }
